"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.isNameOfCloudFormationIntrinsic=exports.minimalCloudFormationJoin=exports.CLOUDFORMATION_TOKEN_RESOLVER=exports.CloudFormationLang=void 0;const lazy_1=require("../lazy"),resolvable_1=require("../resolvable"),stack_1=require("../stack"),token_1=require("../token"),cfn_utils_provider_1=require("./cfn-utils-provider"),resolve_1=require("./resolve");class CloudFormationLang{static toJSON(obj,space){return lazy_1.Lazy.uncachedString({produce:ctx=>tokenAwareStringify(obj,space!=null?space:0,ctx)})}static concat(left,right){if(left===void 0&&right===void 0)return"";const parts=new Array;return left!==void 0&&parts.push(left),right!==void 0&&parts.push(right),parts.length===1?parts[0]:parts.length===2&&isConcatable(parts[0])&&isConcatable(parts[1])?`${parts[0]}${parts[1]}`:fnJoinConcat(parts)}}exports.CloudFormationLang=CloudFormationLang;function fnJoinConcat(parts){return{"Fn::Join":["",minimalCloudFormationJoin("",parts)]}}function tokenAwareStringify(root,space,ctx){let indent=0;const ret=new Array;switch(recurse(ctx.resolve(root,{allowIntrinsicKeys:!0})),ret.length){case 0:return;case 1:return renderSegment(ret[0]);default:return fnJoinConcat(ret.map(renderSegment))}function recurse(obj){if(obj!==void 0){if(token_1.Token.isUnresolved(obj))throw new Error("This shouldnt happen anymore");if(Array.isArray(obj))return renderCollection("[","]",obj,recurse);if(typeof obj=="object"&&obj!=null&&!(obj instanceof Date)){if(isIntrinsic(obj)&&resolve_1.resolvedTypeHint(obj)){renderIntrinsic(obj);return}return renderCollection("{","}",definedEntries(obj),([key,value])=>{key.startsWith(resolve_1.INTRINSIC_KEY_PREFIX)&&([key,value]=value),recurse(key),pushLiteral(prettyPunctuation(":")),recurse(value)})}pushLiteral(JSON.stringify(obj))}}function renderCollection(pre,post,xs,each){pushLiteral(pre),indent+=space;let atLeastOne=!1;for(const[comma,item]of sepIter(xs))comma&&pushLiteral(","),pushLineBreak(),each(item),atLeastOne=!0;indent-=space,atLeastOne&&pushLineBreak(),pushLiteral(post)}function renderIntrinsic(intrinsic){switch(resolve_1.resolvedTypeHint(intrinsic)){case resolve_1.ResolutionTypeHint.STRING:pushLiteral('"'),pushIntrinsic(deepQuoteStringLiterals(intrinsic)),pushLiteral('"');return;case resolve_1.ResolutionTypeHint.LIST:const stack=stack_1.Stack.of(ctx.scope),stringifyResponse=stringifyCache.obtain(stack,JSON.stringify(intrinsic),()=>cfn_utils_provider_1.CfnUtils.stringify(stack,`CdkJsonStringify${stringifyCounter++}`,intrinsic));pushIntrinsic(stringifyResponse);return;case resolve_1.ResolutionTypeHint.NUMBER:pushIntrinsic(intrinsic);return}throw new Error(`Unexpected type hint: ${resolve_1.resolvedTypeHint(intrinsic)}`)}function pushLiteral(lit){let last=ret[ret.length-1];(last==null?void 0:last.type)!=="literal"&&(last={type:"literal",parts:[]},ret.push(last)),last.parts.push(lit)}function pushIntrinsic(intrinsic){ret.push({type:"intrinsic",intrinsic})}function pushLineBreak(){space>0&&pushLiteral(`
...
//# sourceMappingURL=cloudformation-lang.js.map
