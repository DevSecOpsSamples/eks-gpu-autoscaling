...
DCGM_FI_DEV_GPU_UTIL{exported_container="gpu-api"} 로 저장되는 데이터를 DCGM_FI_DEV_GPU_UTIL_AVG{service="gpu-api"} 형태로 조회 할 수 있도록 label을 override합니다
...
```gpu-api``` service, DCGM_FI_DEV_GPU_UTIL_```AVG``` metric 조회
